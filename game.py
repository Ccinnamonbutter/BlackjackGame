...
class Game:
    ...
    def place_bet(self):
        while True:
            bets= float(input("Place your bets: "))
            if bets > self.player.balance :
                print ("You have insufficient funds.Please Try again")
            elif bets < self.MINIMUM_BET :
                print  (f"The minimum bet is $ {self.MINIMUM_BET}. ")
            else:
                self.bet = bets
                self.player.balance -= bets #this line of code subtracts the input bets from the player's balance.
